/*
 * Borrowed the file from https://github.com/woofwoofinc/rust-nova-sds011/
 */
...
fn read_bytes(port: &mut dyn SerialPort) -> io::Result<[u8; 10]> {
    let mut buf = [0_u8; 10];
    println!("read_bytes");

    port.read_exact(buf.as_mut())?;
    println!("{:?}", buf);
    Ok(buf)
}
